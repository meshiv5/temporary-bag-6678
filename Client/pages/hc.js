    ...
    <Box borderRadius={"50px"} w="100%" m="auto" my="50px">
      <Grid backgroundColor={"#FFFFFF"} p="50px" templateColumns={{ base: "repeat(1,1fr)", sm: "repeat(2,1fr)", md: "repeat(3,1fr)" }} gap={5}>
        {helpCenter.map((item, i) => {
          return (
            <GridItem w="300px" h="350px" borderRadius={"20px"} backgroundColor={"#FFFFFF"} border="2px solid #0F0617" key={i}>
              <Text borderTopRadius={"15px"} fontSize={"2xl"} color={"#FFFFFF"}>
                {item.head}
              </Text>
              <Box backgroundColor={"#FFFFFF"} p="10px" pb="50px">
                <Text fontSize={"2xl"} backgroundColor={"#FFFFFF"}>
                  {item.subhead1}
                </Text>
                <Text fontSize={"2xl"} backgroundColor={"#FFFFFF"}>
                  {item.subhead2}
